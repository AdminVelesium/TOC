...
const EmployerProfileSetup2 = () => {
  ...
  const [documentMetas, setDocumentMetas] = useState([]); // <-- new state for meta
  const documentUploadProps = {
    multiple: true,
    fileList: documentFiles,
    beforeUpload: file => {
      setDocumentFiles(prev => [...prev, file]);
      setDocumentMetas(prev => [...prev, { name: file.name, status: "pending", color: "#faad14" }]);
      return false;
    },
    onRemove: file => {
      setDocumentFiles(prev => prev.filter(f => f.uid !== file.uid));
      setDocumentMetas(prev => prev.filter((meta, idx) => documentFiles[idx].uid !== file.uid));
    },
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Ensure meta and files are in sync
      if (documentFiles.length !== documentMetas.length) {
        message.error("Mismatch between uploaded documents and their meta info. Please re-upload.");
        return;
      }
      // Ensure required fields
      const companyName = form.getFieldValue("companyName");
      const companyWebsite = form.getFieldValue("companyWebsite");
      ...
      const data = {
        ...
        employerId: localStorage.getItem('employerId') || undefined, // Attach employerId
      };
      ...
      // Append document files in order
      documentFiles.forEach(file => {
        formData.append("documents", file);
      });
      // Debug log
      console.log("Submitting company profile:", data);
      const response = await fetch('https://toc-bac-1.onrender.com/api/company-profile-setup', {
        method: "POST",
        body: formData,
      });
      let result = {};
      ...
      if (!response.ok) {
        // Specific error for duplicate company
        if (result && result.error && result.error.toLowerCase().includes("already exists")) {
          message.error("A company with this name or website already exists. Please use a different name/website.");
        } else if (result && result.error) {
          message.error(result.error);
        } else {
          message.error("Failed to save company profile. Please check your details and try again.");
        }
        return;
      }
      message.success("Company profile setup completed!");
      navigate("/employer-signin"); // Go to login after profile setup
    } catch (error) {
      ...
      <Card style={{ ...cardStyle, marginBottom: "0" }}>
        ...
        >
          ...
          <TabPane tab="Legal & Contacts" key="legal">
            ...
            >
              ...
              <Col xs={24} lg={10}>
                ...
                <Card title="Legal Documents" style={cardStyle}>
                  ...
                  {documentMetas.map((doc) => (
                    <div key={doc.name} style={documentItemStyle}>
            ...
            {/* Action Buttons only for Legal & Contacts */}
            <div
              style={{
                display: "flex",
                justifyContent: "flex-end",
